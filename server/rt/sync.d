...
version(Windows)
{
    ...
    // import core.atomic: atomicStore, atomicFetchAdd, atomicFetchSub;
    import atomic = rt.atomic;
...
struct Semaphore
{
    ...
    void create(int initc = 0, int maxc = 0xffFF)
    {
        ...
        {
            atomic.cas(&id.count, id.count, initc);
	...
	void signal()
    {
        ...
        else version(Posix)
        {
            ...
            // id.count = (atomicFetchAdd(id.count, 1) + 1);
            atomic.increment(&id.count);
	...
	void wait()
    {
        ...
        else version(Posix)
        {
            ...
            // id.count = (atomicFetchSub(id.count, 1) - 1);
            atomic.decrement(&id.count);
