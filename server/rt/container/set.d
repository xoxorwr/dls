...
struct SparseSet(SparseT)
{
    ...
    {
        sparse.create(allocator);
        dense.create(allocator);
    ...
    void dispose()
    {
        foreach(ref Optional!(SparseT[])  it; sparse)
        {
            if (it.defined)
            {
    ...
    SparseT[] assure(size_t pos)
    {
        if (pos >= sparse.length()) {
            ...
            for(auto i = start_pos; i < sparse.capacity; i ++)
                sparse.items[i] = Optional!(SparseT[]).undefined;
        }

        if(sparse.get(pos).defined == false)
        {
            //printf("alloc bad %llu\n", pos);
            auto new_page = sparse.allocator.alloc!(SparseT)(page_size);
