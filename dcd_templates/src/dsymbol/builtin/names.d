...
/**
 * Name of the DSymbol a `delegate`/`function` type suffix builds.
 *
...
/**
 * Name of the DSymbol an array type suffix (`[]`, `[n]`) builds.
 *
...
/**
 * Name of the DSymbol an associative array type suffix (`[K]`) builds.
 *
...
/**
 * Name of the DSymbol a pointer type suffix (`*`) builds.
 *
 ...
 * in this example). Binds are resolved in the second pass: the lookup carries
 * the bound name and the rename flag (`TypeLookup.selectiveImportName` /
 * `selectiveImportRenamed`).
 *
...
/**
 * Name of the DSymbol an array literal or array initializer builds.
 *
 ...
 *
 * Its child type is the first item's type; an empty array resolves the child
 * from the built-in `void`.
 */
