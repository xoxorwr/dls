...
public:
	...
	/**
	 * The first `return` expression of the function this symbol declares.
	 *
	 * An `auto` function has no declared type at all, so the `return`
	 * expression is the only place its return type appears; `first.d` records
	 * it while it walks the body and turns it into an initializer lookup
	 * (the same path `auto x = <expr>` variables use).
	 */
	Expression autoReturnExpression;
