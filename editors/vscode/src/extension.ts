...
const CREATE_CONFIG_COMMAND = 'dls.createConfig';
const CONFIG_PROMPT_DISMISSED = 'dls.createConfigPromptDismissed';
...
async function maybeSuggestCreateConfig(
  ...
  const choice = await vscode.window.showInformationMessage(
    'dls: no dls.json found in this workspace. Create one to set import paths and flags?',
    create,
    never,
  );
  if (choice === create) {
...
// bracket pairs); this re-applies it at runtime so `dls.autoCloseBrackets`
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration('dls.autoCloseBrackets')) {
        applyLanguageConfiguration();
      }
    }),
  );
  ...
  const config = vscode.workspace.getConfiguration('dls');
  ...
  try {
    ...
    void vscode.window.showErrorMessage(
      `dls: ${err instanceof Error ? err.message : String(err)}`,
    );
    return;
  }
  ...
  client = new LanguageClient('dls', 'dls', serverOptions, clientOptions);
