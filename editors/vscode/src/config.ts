...
const IMPORT_CANDIDATES = ['src', 'source'];
...
export async function createDlsJson(): Promise<boolean> {
  const folder = vscode.workspace.workspaceFolders?.[0];
  if (!folder) {
    void vscode.window.showErrorMessage('dls: open a folder or workspace first.');
    return false;
  }
  ...
  if (fs.existsSync(file)) {
    await open(file);
    void vscode.window.showInformationMessage('dls: dls.json already exists.');
    return false;
  }
  ...
  await open(file);
  void vscode.window.showInformationMessage('dls: created dls.json.');
  return true;
}
